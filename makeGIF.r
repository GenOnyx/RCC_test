...
library(data.table)
#####################################################################################
LevelMarkers = function(geneName, outputFile, path){
  featureSel = geneName
  gpath = getwd()
  
  if(dir.exists(paste0(path, "/", geneName))){
    #setwd(paste0(path, "/", geneName))
  } else {
    system(paste0('mkdir ', path, "/", geneName))
    #setwd(paste0(path, "/", geneName))
  }
  ...
  info = readRDS(outputFile)
  cnum = grep("Clusters", colnames(info))
  gnum = grep(geneName, colnames(info))
  ...
  system(paste0("mv *_", geneName,  ".png ./", geneName, "/"))
  makeGIF(geneName, outputFile, gpath)
}

####################################################################################

makeGIF = function(geneName, outputFile, path){
  if(dir.exists(paste0(path, "/", geneName))){
    gPath = getwd()
    setwd(paste0(path, "/", geneName))
    system(paste0('convert -delay 50 -loop 0 *.png ', geneName, '.gif'))
    setwd(gPath)
  } else {
    LevelMarkers(geneName, outputFile, path)
    print("gene directory does not exist!! Level wise png files created")
  }
}
