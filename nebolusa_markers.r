...
png("threeGene_level1.png", width = 4000, height = 4000)
plot_density(data, reduction = "numap", pal = "inferno",
             features = c("HAMP", "TFRC", "FTL"), joint = T)
dev.off()
...
for(counts in 1:nrow(coordsF)){
  ...
  png(paste0("threeGene_", counts + 1,".png"), 
      width = 4000, height = 4000)
  print(plot_density(data, reduction = "numap", pal = "inferno",
               features = c("HAMP", "TFRC", "FTL"), joint = T))
  dev.off()
}
