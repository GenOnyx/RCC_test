DEG = function(fileName, infoFile, genesFile, outputDir){
  ...
  mat = data
  #mat = t(mat)
  ord_mat = mat[, order(colnames(mat))]
  ord_info = info[order(rownames(info)),]

  identical(colnames(ord_mat), rownames(ord_info))
  #ord_mat = t(ord_mat)
  ...
  for(li in 1:length(Levels)){
    ...
    if(li == 1){
      ...
      for(i in 1:length(a)){
        ...
        lfc1 = apply(lfc_mat, 1, max)
        clusterInfo = cbind(clusterInfo, lfc)
        clusterInfo = cbind(clusterInfo, lfc1)
...
trackingPlot = function(exprsFile, clusterFile, geneFile, attribute, outputDir){
  ...
  DEG(exprsFile, clusterFile, geneFile, outputDir)
